...
TARGETS = [
    "odroid-go"
]  # We just need to specify the default, the others are discovered below
...
)  # os.path.basename(os.getcwd()).title()
PROJECT_ICON = os.getenv("PROJECT_ICON", "icon.raw")
PROJECT_APPS = {}
try:
    PROJECT_VER = (
        os.getenv("PROJECT_VER")
...
def build_firmware(output_file, apps, fw_format="odroid-go", fatsize=0):
    ...
    for app in apps:
        ...
        args += [
            ...
            str(part[2]),
            app,
            os.path.join(app, "build", app + ".bin"),
        ]
...
def build_image(output_file, apps, img_format="esp32", fatsize=0):
    ...
    for app in apps:
        ...
        table_csv.append(
            "%s, app, ota_%d, %d, %d" % (app, table_ota, len(image_data), part_size)
        )
        table_ota += 1
    ...
    bootloader_file = os.path.join(
        os.getcwd(), list(apps)[0], "build", "bootloader", "bootloader.bin"
    )
...
def build_app(
    app, device_type, with_profiling=False, no_networking=False, is_release=False
):
    ...
    args.append(
        f"-DRG_BUILD_TARGET=RG_TARGET_{re.sub(r'[^A-Z0-9]', '_', device_type.upper())}"
    )
    ...
    with open("partitions.csv", "w") as f:
        f.write(
            "# This table isn't used, it's just needed to avoid esp-idf build failures.\n"
        )
...
def flash_app(app, port, baudrate=1152000):
    ...
    if not os.path.exists("partitions.bin"):
        ...
        run(
            [ESPTOOL_PY, "read_flash", "0x8000", "0x1000", "partitions.bin"],
            check=False,
        )
...
parser.add_argument(
    "--fatsize",
    help="Add FAT storage partition of provided size (500K, 5M,...) to the built image.",
)
...
if os.path.exists(f"components/retro-go/targets/{args.target}/sdkconfig"):
    os.putenv(
        "SDKCONFIG_DEFAULTS",
        os.path.abspath(f"components/retro-go/targets/{args.target}/sdkconfig"),
    )
...
try:
    if (
        command in ["build-fw", "build-img", "release", "install"]
        and "launcher" not in apps
    ):
        print(
    ...
    if command in ["build-img", "release", "install"]:
        ...
        build_image(
            img_file,
            apps,
            os.getenv("IMG_FORMAT", os.getenv("IDF_TARGET")),
            args.fatsize,
        )
