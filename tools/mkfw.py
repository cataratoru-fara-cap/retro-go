...
if len(sys.argv) < 9:
    exit(
        "usage: mkfw.py [--esplay] output_file.fw 'description' tile.raw type subtype size label file.bin "
...
while len(args) >= 5:
    ...
    print(
        "[%d]: type=%d, subtype=%d, size=%d (%d%% used), label=%s"
        % (fw_part, partype, subtype, real_size, usage, label)
    )

    if real_size > size and size != 0:
        print(
            " > WARNING: Partition smaller than file (+%d bytes), increasing size to %d"
            % (len(data) - size, real_size)
        )

    fw_data += struct.pack(
        "<BBxx16sIII", partype, subtype, label.encode(), 0, real_size, len(data)
    )
    fw_data += data
    fw_size += real_size
    fw_part += 1
